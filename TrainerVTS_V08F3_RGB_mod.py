...
class ImageDecoder(nn.Module):
    ...
    def __init__(self, batchnorm='identity', latent_dim=16, active_func=nn.Sigmoid(), *args, **kwargs):
        ...
        cnn = []
        cnn.extend([nn.Conv2d(6, 512, 1, 1, 0), nn.LeakyReLU(inplace=True)])
        # cnn.extend([nn.Conv2d(in_channels=6, out_channels=6, kernel_size=3, padding=1, groups=6)])# Depthwise
        # cnn.extend([nn.Conv2d(6, 128, kernel_size=1)])  # Pointwise
        # cnn.extend([nn.Conv2d(128, 512, kernel_size=1)])  # Pointwise
        ...
        self.fclayers = nn.Sequential(
            nn.Linear(self.latent_dim, 6 * 16 * 16),
        )
    ...
    def forward(self, x):
        out = self.fclayers(x)
        out = self.cnn(out.view(-1, 6, 16, 16))
        return out.view(-1, 1, 128, 128)
    

class NCC_MSE_Loss():
    def __init__(self, lambda_ncc=0.7, epsilon=1e-10):
    ...
    def ncc(self, pred, target):
        ...
        mean_target = torch.mean(target, dim=(1, 2, 3), keepdim=True)

        numerator = torch.sum((pred - mean_pred) * (target - mean_target), dim=(1, 2, 3))
        denominator = torch.sqrt(
            torch.sum((pred - mean_pred) ** 2, dim=(1, 2, 3)) * 
            torch.sum((target - mean_target) ** 2, dim=(1, 2, 3))
        )
...
class TeacherTrainer(BasicTrainer):
    ...
    def calculate_loss(self, data):
        ...
        vae_loss = kl_loss * self.beta * self.latent_weight + r_recon_loss * self.rimg_weight + c_recon_loss * self.cimg_weight
        ...
        TEMP_LOSS = {'LOSS': loss,
                    'KL': kl_loss * self.latent_weight,
                    'R_RECON': r_recon_loss * self.rimg_weight,
                    'C_RECON': c_recon_loss * self.cimg_weight,
