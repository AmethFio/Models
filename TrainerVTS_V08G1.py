...
class CSIEncoder(BasicCSIEncoder):
    def __init__(self, lstm_steps=steps, lstm_feature_length=feature_length, *args, **kwargs):
        ...
        self.fc_mu = nn.Sequential(
            nn.Linear(self.feature_length, self.attention_length),
            nn.ReLU(),
            SelfAttention(self.attention_length),
            nn.Linear(self.attention_length, self.latent_dim)
        )
...
if __name__ == '__main__':
    cc = CSIEncoder(latent_dim=128, lstm_steps=75).to(torch.device('cuda:7'))
    summary(cc, input_size=((6, 30, 300), (1, 62)))
