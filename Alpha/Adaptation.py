...
class NCC_MSE_Loss(torch.nn.Module):
    ...
    def ncc(self, pred, target):
        ...
        numerator = torch.sum((pred - mean_pred) * (target - mean_target), dim=-1)
        denominator = torch.sqrt(
            torch.sum((pred - mean_pred) ** 2, dim=-1) * 
            torch.sum((target - mean_target) ** 2, dim=-1)
        )
