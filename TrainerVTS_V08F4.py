...
class TeacherTrainer(BasicTrainer):
    def __init__(self,
        ...
        self.beta = beta
        self.rimg_recon_loss = nn.MSELoss(reduction='sum')
        self.cimg_recon_loss = nn.BCELoss(reduction='sum')
        self.with_correlation = with_correlation

        self.loss_terms = ('LOSS', 'KL', 'R_RECON', 'C_RECON', 'R_CORR', 'C_CORR', 'CTR', 'DPT')
    ...
    def correlation_loss(self, pred, target):
        # Subtract mean from predicted and true values
        pred = pred.reshape(pred.shape[0], -1)
        target = target.reshape(target.shape[0], -1)
        
        pred_centered = pred - torch.mean(pred)
        true_centered = target - torch.mean(target)
        
        # Calculate covariance between y_pred and y_true
        covariance = torch.sum(pred_centered * true_centered)
        
        # Calculate standard deviations
        std_pred = torch.sqrt(torch.sum(pred_centered ** 2))
        std_true = torch.sqrt(torch.sum(true_centered ** 2))
        
        # Calculate correlation coefficient
        correlation = covariance / (std_pred * std_true)
        
        # Calculate loss (1 - correlation)
        loss = 1 - correlation
    ...
    def calculate_loss(self, data):
        ...
        r_recon_loss = self.rimg_recon_loss(rimg_re, rimg) / rimg_re.shape[0]
        c_recon_loss = self.cimg_recon_loss(cimg_re, cimg) / cimg_re.shape[0]
        r_corr_loss = self.correlation_loss(rimg_re, rimg)
        c_corr_loss = self.correlation_loss(cimg_re, cimg)
...
class StudentTrainer(BasicTrainer):
    def __init__(self,
                 alpha=0.8,
                 rimg_loss=nn.MSELoss(reduction='sum'),
                 with_correlation=False,
                 lstm_steps=7,
                 *args, **kwargs):
        ...
        self.alpha = alpha
        self.rimg_recon_loss = rimg_loss
        self.cimg_recon_loss = nn.BCELoss(reduction='sum')
        self.with_correlation = with_correlation
        ...
        self.loss_terms = ('LOSS', 'LATENT', 'MU', 'LOGVAR', 'FEATURE', 'RIMG', 'R_CORR', 'C_CORR', 'CTR', 'DPT')
    ...
    def calculate_loss(self, data):
        ...
        image_loss = self.rimg_recon_loss(s_rimage, rimg)
        r_corr_loss = self.correlation_loss(s_rimage, rimg)
        c_corr_loss = self.correlation_loss(s_cimage, cimg)
