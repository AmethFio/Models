...
class StudentTrainer(BasicTrainer):
    ...
    def coord_sim_loss(self, pred_coord, gt_coord):
        # Normalize the vectors for cosine similarity
        pred_coord = pred_coord / pred_coord.norm(dim=0, keepdim=True)
        gt_coord = gt_coord / gt_coord.norm(dim=0, keepdim=True)

        # Compute cosine similarity matrix (shape: (4,4))
        cos_sim = torch.matmul(pred_coord[:, None], gt_coord[None, :])

        # Create target labels: 1 for same index (positive pairs), 0 for others (negative pairs)
        target = torch.eye(len(pred_coord))  # Identity matrix (4x4) with 1s on the diagonal
