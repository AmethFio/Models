...
class TeacherTrainer(BasicTrainer):
    def __init__(self,
        ...
        self.modality = {'rimg', 'cimg', 'tag'}

        self.beta = beta
        self.recon_lossfunc = recon_lossfunc

        self.loss_terms = ('LOSS', 'KL', 'R_RECON', 'C_RECON')
        self.pred_terms = ('R_GT', 'C_GT', 'R_PRED', 'C_PRED', 'LAT', 'TAG')
    ...
    def calculate_loss(self, data):
        ...
        z, mu, logvar, _ = self.models['imgen'](rimg)
        ...
        loss = kl_loss * self.beta + r_recon_loss + c_recon_loss
...
class StudentTrainer(BasicTrainer):
    def __init__(self,
        ...
        self.loss_terms = ('LOSS', 'LATENT', 'MU', 'LOGVAR', 'FEATURE', 'CTR', 'DPT', 'IMG')
    ...
    def calculate_loss(self, data):
        ...
        if self.with_img_loss:
            image_loss = self.recon_lossfunc(s_rimage, t_rimage)
