...
class TeacherTrainer(BasicTrainer):
    ...
    def update_beta(self):
        if self.current_ep() > 100:
            self.beta = min(self.max_beta, 
                            self.initial_beta + (self.max_beta / self.annealing_epochs) * (self.current_ep() - 100))
...
class StudentTrainer(BasicTrainer):
    ...
    def calculate_loss(self, mode, data2):
        ...
        def s_losses(s_out, t_out, data):
            ...
            return {
                'LOSS'   : loss,
                'MU'     : mu_loss * self.latent_weight * self.alpha,
                'LOGVAR' : logvar_loss * self.latent_weight * (1 - self.alpha),
