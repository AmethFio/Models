...
class TeacherTrainer(BasicTrainer):
    def __init__(self,
        ...
        self.loss_terms = ('LOSS', 'KL', 'R_RECON', 'C_RECON', 'CTR', 'DPT', 'P_DPT')
    ...
    def calculate_loss(self, data):
        ...
        #post_depth = torch.sum(rimg_re, dim=(-1, -2)) / torch.sum(rimg_re != 0, dim=(-1, -2))
        post_depth = torch.mean(rimg_re[rimg_re>0], dim=-1)
        
        center_loss = self.center_loss(ctr, torch.squeeze(data['center']))
        depth_loss = self.depth_loss(depth, torch.squeeze(data['depth']))
        post_depth_loss = self.depth_loss(post_depth, torch.squeeze(data['depth']))
        
        loss = vae_loss * 0.001 + center_loss + depth_loss + post_depth_loss
...
class StudentTrainer(BasicTrainer):
    def __init__(self,
        ...
        self.depth_loss = nn.MSELoss()
        self.center_loss = nn.MSELoss()
        self.with_post_depth_loss = with_post_depth_loss
        
        self.mse = nn.MSELoss()
        self.feature_loss = nn.MSELoss()
        self.rimg_loss = rimg_loss

        self.loss_terms = ('LOSS', 'LATENT', 'MU', 'LOGVAR', 'FEATURE', 'IMG', 'CTR', 'DPT', 'P_DPT')
        ...
        self.losslog.dpt = ['GT_DPT', 'T_DPT', 'S_DPT', 'P_DPT']
    ...
    def calculate_loss(self, data):
        ...
        post_depth = torch.sum(s_rimage, dim=(-1, -2)) / torch.sum(s_rimage != 0, dim=(-1, -2))
        ...
        if self.with_post_depth_loss:
            post_depth_loss = self.depth_loss(post_depth, torch.squeeze(data['depth']))
            loss += post_depth_loss
        else:
            with torch.no_grad():
                post_depth_loss = self.depth_loss(post_depth, torch.squeeze(data['depth']))
